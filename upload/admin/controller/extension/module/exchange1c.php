...
// htmlImage()
    ...
    private function htmlInput($name, $param, $type = 'text') {
        ...
        if (empty($value) && !empty($param['default']))
            $value = $param['default'];
...
// uninstall()
    ...
    private function checkAccess($echo = false) {
        ...
        if (!$this->config->get('exchange1c_module_status')) {
            if ($echo)
                $this->echo_message(0, "The module is disabled");
            return false;
        }
        // Разрешен ли IP
        ...
        if ($echo)
            $this->echo_message(0, "From Your IP address are not allowed");
        return false;
    }
...
// manualImport()
    ...
    private function checkAuthKey($echo = true) {
        ...
        if (!isset($this->request->cookie['key'])) {
            if ($echo)
                $this->echo_message(0, "no cookie key\n");
            return false;
        }
        if ($this->request->cookie['key'] != md5($this->config->get('exchange1c_password'))) {
            if ($echo)
                $this->echo_message(0, "Session error\n");
            return false;
        }
        return true;
    }
...
// modeFileSale()
    ...
    public function modeImport($manual = false) {
        ...
        elseif (isset($this->request->get['filename']))
            ...
            if (!$manual)
                $this->echo_message(0, "No import file name");
